...
public class TaskServiceTest extends Assert {
    ...
    public void prepare() {
        ...
        // добавление новой задачи в репозиторий при создании
        ...
        // получение задачи по id из репозитрия
        ...
        // передаем моки в реализацию интерфейса TaskService
        taskService = new TaskServiceImpl(taskRepository, userService);
    }

    @Test
    public void CRUDTest() {
        String rawData = "Задача 1_Описание 1_ilyabaykalov";

        // добавили в репозиторий
        taskService.addTask(taskService.parseTask(rawData));

        // чтение задачи по id из репозитория
        Task taskFromService = taskService.getTask(anyInt());

        assertEquals(taskFromService.getTaskTitle(), "Задача 1");

        // смена статуса задачи
        taskService.setStatus(taskFromService.getId(), "FINISHED");
        assertEquals(taskFromService.getStatus(), FINISHED);

        taskFromService.setTaskTitle("Изменил название задачи 1");
        taskFromService.setTaskDescription("Изменил описание ещё задачи 1");

        // обновление задачи
        taskService.editTask(taskFromService);

        int id = taskFromService.getId();
        ...
        // объект удален из репозитория
        Task anotherTaskFromService = taskService.getTask(id);
        assertNull(anotherTaskFromService);
    }
